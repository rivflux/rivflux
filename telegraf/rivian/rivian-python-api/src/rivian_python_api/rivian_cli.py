...
def main():
    ...
    if args.poll or args.query or args.all:
        ...
        print(f"timestamp,Cabin Temp,Power,Drive Mode,Gear,Mileage,Battery,Range,{lat_long_title}Charger Status,Charge State,Battery Limit,Charge End,Charge Elapsed,Charge Power,Charge Session")
        ...
        while True:
            ...
            if not state:
                if not found_bad_response:
                    print(f"# {datetime.datetime.now(datetime.timezone.utc).isoformat()} Rivian API appears offline")
            ...
            current_state = \
                f"{state['cabinClimateInteriorTemperature']['value']}," \
            ...
            if state['chargerState']['value'] == "charging_active":
                s = live_charging_session(vehicle_id=vehicle_id, verbose=args.verbose)
                current_state += \
                f"{s['timeElapsed']}," \
                f"{s['power']['value']}," \
                f"{s['totalChargedEnergy']['value']}"
            else:
                current_state += "0,0,0"
            if args.poll_show_all or single_poll or current_state != last_state:
                print(f"{datetime.now(timezone.utc).isoformat()}," + current_state)
